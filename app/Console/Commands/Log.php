...
class Log extends Command
{
     ...
     * @var string
     */
    protected $signature = 'rabbitmq:log';
     ...
     * @var string
     */
    protected $description = 'RabbitMq Consumer : LogQueue';

    protected $queueName = Util::LOG_QUEUE;
    ...
    public function handle()
    {
        ...
        $callback = function ($msg) {
            ...
            try {
                ...
                $dossier = "/var/www/html/storage/request_logs/".gmdate("Y-m-d")."/";
                if(!empty($data["fileName"])){
                    $nomfichier = $data["fileName"]. ".log";
                }else{
                    $prefix = !empty($data["fileNamePrefix"]) ? $data["fileNamePrefix"] : "";
                    $nomfichier = $prefix.gmdate("YmdHis") . ".log";
                }
                if (!is_dir($dossier)) {
                    mkdir($dossier, 0777, true);
                }
                $fichier = fopen($dossier . $nomfichier, "a+");
                fputs($fichier, json_encode($data));
                fclose($fichier);

            }catch (Exception $exception){
                echo ' [x] '.gmdate("Y-m-d H:i:s").' Error. details : ', $msg->body, "\n";
            }

      ...
      <div class="card border-0 mb-4">
        <div class="card-body bg-info-subtle rounded">
          <div class="row align-items-center">
            ...
            <div class="col-8">
              <h3 class="font-weight-bold mb-0">{{ number_format($viewData["userStats"]['total_vuesmoyen'] ?? 0) }}</h3>
      ...
      <div class="card border-0 mb-4">
        <div class="card-body bg-warning-subtle rounded">
          <div class="row align-items-center">
            ...
            <div class="col-8">
              <h3 class="font-weight-bold mb-0">{{ number_format($viewData["taskStats"]['total_budget'] ?? 0) }} F</h3>
              ...
              <div class="col-6">
                ...
                <div class="progress" style="height: 8px;">
                  <div class="progress-bar bg-warning" role="progressbar" 
                       style="width: {{ ($viewData["taskStats"]['total'] > 0) ? (($viewData["taskStats"]['pending'] ?? 0) / $viewData["taskStats"]['total'] * 100) : 0 }}%"></div>
                </div>
              </div>
              <div class="col-6">
                <div class="d-flex justify-content-between">
                ...
                <div class="progress" style="height: 8px;">
                  <div class="progress-bar bg-primary" role="progressbar" 
                       style="width: {{ ($viewData["taskStats"]['total'] > 0) ? (($viewData["taskStats"]['accepted'] ?? 0) / $viewData["taskStats"]['total'] * 100) : 0 }}%"></div>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-6">
                ...
                <div class="progress" style="height: 8px;">
                  <div class="progress-bar bg-success" role="progressbar" 
                       style="width: {{ ($viewData["taskStats"]['total'] > 0) ? (($viewData["taskStats"]['paid'] ?? 0) / $viewData["taskStats"]['total'] * 100) : 0 }}%"></div>
                </div>
              </div>
              <div class="col-6">
                <div class="d-flex justify-content-between">
                ...
                <div class="progress" style="height: 8px;">
                  <div class="progress-bar bg-danger" role="progressbar" 
                       style="width: {{ ($viewData["taskStats"]['total'] > 0) ? (($viewData["taskStats"]['rejected'] ?? 0) / $viewData["taskStats"]['total'] * 100) : 0 }}%"></div>
                </div>
              </div>
            </div>
          </div>
            ...
            <table class="table mb-0">
              ...
              <tbody>
                @if(isset($viewData["recentTasks"]) && count($viewData["recentTasks"]) > 0)
                  ...
                  <tr>
                    <td>
                      <a href="{{ route('admin.task', ['id' => $task->id]) }}">{{ $task->name }}</a>
                    </td>
    ...
    <div class="col-lg-6">
      <div class="card mb-4">
        <div class="card-header d-flex justify-content-between">
          ...
          <a href="{{ route('admin.users', ['group' => 'all']) }}" class="btn btn-sm btn-outline-primary">Voir tout</a>
        </div>
        <div class="card-body p-0">
          <div class="table-responsive">
            <table class="table mb-0">
    ...
    <div class="col-md-4">
      <div class="card mb-4">
        <div class="card-body">
          ...
          <div class="mt-3">
            <a href="{{ route('admin.users', ['group' => 'all']) }}" class="btn btn-primary btn-sm w-100">Accéder</a>
          </div>
        </div>
      </div>
    </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var taskStatusChart = new Chart(taskCtx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: [
            {{ $viewData["taskStats"]['pending'] ?? 0 }}, 
            {{ $viewData["taskStats"]['accepted'] ?? 0 }}, 
            {{ $viewData["taskStats"]['paid'] ?? 0 }}, 
            {{ $viewData["taskStats"]['rejected'] ?? 0 }},
            {{ $viewData["taskStats"]['closed'] ?? 0 }}
          ],
          backgroundColor: [
            'rgba(255, 193, 7, 0.7)',
            'rgba(13, 110, 253, 0.7)',
    ...
    var userDistributionChart = new Chart(userCtx, {
      ...
      data: {
        ...
        datasets: [{
          data: [
            {{ $viewData["userStats"]['influencers'] ?? 0 }}, 
            {{ $viewData["userStats"]['announcers'] ?? 0 }}, 
            {{ $viewData["userStats"]['admins'] ?? 0 }}
          ],
          backgroundColor: [
            'rgba(13, 110, 253, 0.7)',
            'rgba(25, 135, 84, 0.7)',

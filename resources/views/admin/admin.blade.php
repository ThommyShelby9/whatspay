...
<div class="container-fluid">
            ...
            <div class="d-flex">
              <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="filterDropdown">
                  <li><a class="dropdown-item {{ request()->route('group') == 'all' ? 'active' : '' }}" href="{{ route('admin.users', ['group' => 'all']) }}">Tous</a></li>
                  <li><a class="dropdown-item {{ request()->route('group') == 'admin' ? 'active' : '' }}" href="{{ route('admin.users', ['group' => 'admin']) }}">Administrateurs</a></li>
                  <li><a class="dropdown-item {{ request()->route('group') == 'annonceur' ? 'active' : '' }}" href="{{ route('admin.users', ['group' => 'annonceur']) }}">Annonceurs</a></li>
                  <li><a class="dropdown-item {{ request()->route('group') == 'diffuseur' ? 'active' : '' }}" href="{{ route('admin.users', ['group' => 'diffuseur']) }}">Diffuseurs</a></li>
                </ul>
              </div>
            </div>
          </div>
  ...
  <div class="row mb-4">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <form action="{{ route('admin.users', ['group' => request()->route('group')]) }}" method="GET" class="row g-3">
            <div class="col-md-3">
            ...
            <table class="table table-hover table-striped">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  @endif
                  <td>
                    <form action="{{ route('admin.users.update', ['group' => request()->route('group')]) }}" method="POST" class="toggle-status-form">
                      @csrf
                      <input type="hidden" name="user_id" value="{{ $user->id }}">
                      <input type="hidden" name="action" value="toggle_status">
                      <div class="form-check form-switch">
                        <input class="form-check-input toggle-status" type="checkbox" role="switch" 
                          name="enabled" value="1" onchange="this.form.submit()"
                  ...
                  <td>
                    <div class="btn-group" role="group">
                      <a href="{{ route('admin.users', ['group' => request()->route('group'), 'action' => 'view', 'id' => $user->id]) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-eye"></i>
                      </a>
                      <a href="{{ route('admin.users', ['group' => request()->route('group'), 'action' => 'edit', 'id' => $user->id]) }}" class="btn btn-sm btn-warning">
                ...
                @empty
                <tr>
                  <td colspan="{{ request()->route('group') == 'diffuseur' ? 9 : 7 }}" class="text-center">Aucun utilisateur trouvé</td>
                </tr>
                @endforelse
              </tbody>
            </table>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                <p>{{ $viewData['userDetails']->firstname }} {{ $viewData['userDetails']->lastname }}</p>
              </div>
              <div class="mb-3">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                <p>{!! $viewData['userDetails']->enabled ? '<span class="badge bg-success">Actif</span>' : '<span class="badge bg-danger">Inactif</span>' !!}</p>
              </div>
              <div class="mb-3">
                <label class="fw-bold">Email vérifié:</label>
                <p>{!! $viewData['userDetails']->email_verified_at ? '<span class="badge bg-success">Oui</span>' : '<span class="badge bg-danger">Non</span>' !!}</p>
              </div>
            </div>
          </div>
          ...
          <div class="d-flex justify-content-end mt-3">
            <a href="{{ route('admin.users', ['group' => request()->route('group')]) }}" class="btn btn-secondary">Retour à la liste</a>
          </div>
        </div>
      </div>
    </div>
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <select class="form-select" id="country_id" name="country_id">
                    @foreach($viewData['countries'] ?? [] as $country)
                    <option value="{{ $country->id }}" {{ $viewData['userDetails']->country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <select class="form-select" id="locality_id" name="locality_id">
                    ...
                    @foreach($viewData['localities'] ?? [] as $locality)
                    <option value="{{ $locality->id }}" {{ $viewData['userDetails']->locality_id == $locality->id ? 'selected' : '' }}>{{ $locality->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <input type="number" class="form-control" id="vuesmoyen" name="vuesmoyen" value="{{ $viewData['userDetails']->vuesmoyen }}">
                </div>
              </div>
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <select class="form-select" id="lang_id" name="lang_id">
                    ...
                    <option value="{{ $lang->id }}" {{ $viewData['userDetails']->lang_id == $lang->id ? 'selected' : '' }}>{{ $lang->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <select class="form-select" id="study_id" name="study_id">
                    ...
                    <option value="{{ $study->id }}" {{ $viewData['userDetails']->study_id == $study->id ? 'selected' : '' }}>{{ $study->name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>

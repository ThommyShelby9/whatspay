...
@section('pagecontent')
    ...
    <div class="container-fluid">
        ...
        @if (isset($viewData['lowBalanceAlert']))
            <div class="row mb-4">
                <div class="col-12">
                    <div class="alert alert-{{ $viewData['lowBalanceAlert']['type'] }} alert-dismissible fade show"
                        role="alert">
                        <i class="fa fa-exclamation-triangle me-2"></i>
                        {{ $viewData['lowBalanceAlert']['message'] }}
                        @if ($viewData['lowBalanceAlert']['action_required'])
                            <button type="button" class="btn btn-sm btn-light ms-2" data-bs-toggle="modal"
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-6">
                                            ...
                                            <div class="fw-semibold">
                                                {{ number_format($viewData['spendingStats']['total_budget_reserved'] ?? 0, 0, ',', ' ') }}
                                                F</div>
                                        </div>
                                        <div class="col-6">
                                            <small class="text-muted">En attente</small>
                                            <div class="fw-semibold text-warning">
                                                {{ number_format($viewData['spendingStats']['pending_payments'] ?? 0, 0, ',', ' ') }}
                                                F</div>
                                        </div>
                                    </div>
                                @endif
            ...
            <div class="row mb-4">
                    ...
                    <div class="card">
                        <div class="card-body text-center">
                            <h3 class="text-success">
                                {{ number_format($viewData['transactionStats']['this_month_credits'], 0, ',', ' ') }} F</h3>
                    ...
                    <div class="card">
                        <div class="card-body text-center">
                            <h3 class="text-danger">
                                {{ number_format($viewData['transactionStats']['this_month_debits'], 0, ',', ' ') }} F</h3>
                ...
                <div class="col-md-3">
                    <div class="card">
                        <div class="card-body text-center">
                            <h3 class="text-info">
                                {{ isset($viewData['spendingStats']) ? $viewData['spendingStats']['active_campaigns'] : 0 }}
                            </h3>
                                    ...
                                    <tbody>
                                        @foreach ($viewData['transactions'] as $transaction)
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="badge bg-{{ $transaction->type == 'Crédit' ? 'success' : 'primary' }}">
                                                        {{ $transaction->type }}
                                                    </span>
                                                </td>
                                                <td
                                                ...
                                                <td>
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="badge bg-{{ $config['class'] }}">{{ $config['text'] }}</span>
                                                </td>
                                                <td>
                                            ...
                                            <div class="card-body text-center">
                                                ...
                                                @if ($method->config)
                                                    ...
                                                    @if (isset($config['processing_time']))
                                                        <small
                                                            class="text-muted d-block mt-2">{{ $config['processing_time'] }}</small>
                                                    @endif
                                                @endif
                                            </div>
                                        </div>
                                ...
                                @if (!isset($viewData['paymentMethods']) || count($viewData['paymentMethods']) === 0)
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <div class="card payment-method-card">
                                            <div class="card-body text-center">
                                                <input type="radio" class="btn-check" name="payment_method"
                                                    value="mobile_money" id="mobile_money" required>
                            ...
                            <div class="row">
                                @foreach ([100, 500, 1000, 5000, 10000, 25000] as $amount)
                                    <div class="col-md-4 col-sm-6 mb-2">
                                        <button type="button" class="btn btn-light w-100 suggested-amount"
                                            data-amount="{{ $amount }}">
                    ...
                    $.get('{{ route('announcer.wallet.stats') }}')
                        .done(function(data) {
                            if (data.success && data.wallet) {
                                $('#current-balance').text(new Intl.NumberFormat('fr-FR').format(data.wallet
                                    .wallet.balance) + ' F');
                            }
                        })
                        .fail(function() {

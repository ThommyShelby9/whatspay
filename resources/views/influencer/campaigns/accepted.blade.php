                ...
                <div class="row">
                    @php
                        $activeMissions = collect($viewData['assignments'] ?? [])->filter(function ($assignment) {
                            return in_array($assignment->status, ['PENDING']);
                        });
                    @endphp
                        ...
                        <div class="col-xl-4 col-md-6 mb-4">
                            <div class="card h-100">
                                <div class="card-header bg-primary bg-soft">
                                    <h5 class="card-title text-white mb-0">{{ $assignment->task->name ?? 'Sans titre' }}
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <div>
                                            {{-- <span
                                                class="badge bg-primary">{{ number_format($assignment->task->total_views_estomated ?? 0) }}
                                                F</span> --}}
                                            <p class="text-muted mb-0 mt-1">
                                                Date limite:
                                                {{ isset($assignment->task->enddate) ? date('d/m/Y', strtotime($assignment->task->enddate)) : 'N/A' }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3 text-center">
                                        ...
                                        @if ($assignment->status == 'PENDING')
                                            <a href="{{ route('influencer.campaigns.submit', ['id' => $assignment->id]) }}"
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body text-center py-5">
                                    {{-- <img src="{{ asset('images/empty-state.svg') }}" alt="Aucune mission" height="120"
                                        class="mb-4"> --}}
                                    ...
                                    <a href="{{ route('influencer.campaigns.available') }}"
                                        class="btn btn-primary mt-2">Trouver des campagnes</a>
                                </div>
                            </div>
                        </div>
                    @endforelse
                ...
                <div class="row">
                    @php
                        $completedMissions = collect($viewData['assignments'] ?? [])->filter(function ($assignment) {
                            return in_array($assignment->status, ['SUBMITED']);
                        });
                    @endphp
                            ...
                            <div class="card h-100">
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <div>
                                            <span class="badge bg-success">{{ number_format($assignment->gain ?? 0) }}
                                                F</span>
                                            <p class="text-muted mb-0 mt-1">
                                                Terminée le:
                                                {{ isset($assignment->submission_date) ? date('d/m/Y', strtotime($assignment->submission_date)) : 'N/A' }}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <tbody>
                                    @forelse($viewData["assignments"] ?? [] as $assignment)
                                        <tr>
                                            ...
                                            <td>
                                                <div>Début:
                                                    {{ isset($assignment->task->startdate) ? date('d/m/Y', strtotime($assignment->task->startdate)) : 'N/A' }}
                                                </div>
                                                <div>Fin:
                                                    {{ isset($assignment->task->enddate) ? date('d/m/Y', strtotime($assignment->task->enddate)) : 'N/A' }}
                                                </div>

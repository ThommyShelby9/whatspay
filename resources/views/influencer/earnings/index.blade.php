  ...
  <div class="row mb-4">
          ...
          <div class="d-flex align-items-center">
            ...
            <div class="flex-grow-1 ms-4">
              ...
              <h1 class="mb-3 text-success" id="available-balance">
                {{ number_format($viewData["earningsStats"]["available_for_withdrawal"] ?? 0, 0, ',', ' ') }} F
              </h1>
              <div class="row mb-3">
                <div class="col-4">
                  <h6 class="text-muted mb-1">Gains totaux</h6>
                  <h5 class="text-primary">{{ number_format($viewData["earningsStats"]["total_earnings"] ?? 0, 0, ',', ' ') }} F</h5>
                </div>
                <div class="col-4">
                  <h6 class="text-muted mb-1">Ce mois-ci</h6>
                  <h5>{{ number_format($viewData["earningsStats"]["current_month"] ?? 0, 0, ',', ' ') }} F</h5>
                </div>
                <div class="col-4">
                  <h6 class="text-muted mb-1">Mois précédent</h6>
                  <h5>{{ number_format($viewData["earningsStats"]["last_month"] ?? 0, 0, ',', ' ') }} F</h5>
                </div>
              </div>
              <div class="btn-group">
    ...
    <div class="col-md-4">
      <div class="card">
        ...
        <div class="card-body">
          <h4 class="mb-1 text-warning">{{ number_format($viewData["earningsStats"]["pending_payment"] ?? 0, 0, ',', ' ') }} F</h4>
  ...
  @if(isset($viewData['assignmentStats']))
    <div class="row mb-4">
      ...
      <div class="col-md-3">
        <div class="card">
          <div class="card-body text-center">
            <h3 class="text-success">{{ number_format($viewData['assignmentStats']['total_vues'] ?? 0, 0, ',', ' ') }}</h3>
      ...
      <div class="col-md-3">
        <div class="card">
          <div class="card-body text-center">
            @php $avgPerView = ($viewData['assignmentStats']['total_vues'] ?? 0) > 0 ? 3 : 0; @endphp
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          ...
          <div>
            <button type="button" class="btn btn-sm btn-light period-btn active" data-period="6months">6 mois</button>
            <button type="button" class="btn btn-sm btn-light period-btn" data-period="year">1 an</button>
        ...
        <div class="card-body">
                    ...
                    <tr>
                      ...
                      <td>
                        @php
                          $typeConfig = [
                            ...
                            'BUDGET_RESERVE' => ['class' => 'warning', 'icon' => 'lock'],
                            'PLATFORM_COMMISSION' => ['class' => 'info', 'icon' => 'percentage']
                          ];
                          $config = $typeConfig[$transaction->type] ?? ['class' => 'secondary', 'icon' => 'circle'];
                        @endphp
                        <span class="badge bg-{{ $config['class'] }}">
                          <i class="fa fa-{{ $config['icon'] }} me-1"></i>{{ $transaction->type }}
                        </span>
                      </td>
                      ...
                      <td>
                        @php
                          ...
                          $sConfig = $statusConfig[$transaction->status] ?? ['class' => 'secondary', 'text' => $transaction->status];
                        @endphp
          ...
          @else
            <div class="text-center py-4">
              ...
              <p class="text-muted mb-0">Vos premiers gains apparaîtront ici une fois vos campagnes validées.</p>
            </div>
          @endif
        </div>
      </div>
              ...
              <div class="alert alert-info">
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <span class="text-success">{{ number_format($viewData["earningsStats"]["available_for_withdrawal"] ?? 0, 0, ',', ' ') }} F</span>
                  </div>
                  <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                <div class="input-group">
                  <input type="number" class="form-control" name="amount" id="withdrawal-amount"
                         min="500" max="{{ $viewData["earningsStats"]["available_for_withdrawal"] ?? 0 }}" 
                ...
                <div class="form-text">
                  <div class="d-flex justify-content-between">
                    ...
                    <span>Maximum: {{ number_format($viewData["earningsStats"]["available_for_withdrawal"] ?? 0, 0, ',', ' ') }} F</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="mb-4">
            ...
            <div class="row">
              @php
                ...
                $suggestions = [];
                if ($balance >= 500) $suggestions[] = 500;
                if ($balance >= 1000) $suggestions[] = 1000;
                if ($balance >= 5000) $suggestions[] = 5000;
                if ($balance >= 10000) $suggestions[] = 10000;
                if ($balance >= 25000) $suggestions[] = 25000;
                if ($balance > 50000) $suggestions[] = floor($balance / 2);
                if ($balance >= 1000) $suggestions[] = $balance; // Tout retirer
              @endphp
              
              @foreach(array_unique($suggestions) as $amount)
                <div class="col-md-4 col-sm-6 mb-2">
                  <button type="button" class="btn btn-light w-100 suggested-withdrawal-amount" data-amount="{{ $amount }}">
                    @if($amount == $balance)
                      Tout retirer
                    @else
...
$(document).ready(function() {
    ...
    $('#submit-withdrawal').click(function() {
        ...
        if (method === 'bank' && (!$('select[name="bank_name"]').val() || !$('input[name="bank_account"]').val())) {
    ...
    $('[data-export]').click(function(e) {
        ...
        window.location = '{{ route("influencer.earnings.export") }}?format=' + format + '&period=' + period;
    });
    ...
    function updateChart(period) {
        ...
        $.get('{{ route("influencer.earnings.chart-data") }}', { period: period })
